...
const CreateCommittee = () => {
  ...
  async function getCategories() {
    ...
    if (getResult.statusCode === 200) {
      const findedCategory = getResult.data.parents.filter((item) => {
        if (item.name === "کمیته" && item.parent === null) return item;
      });
  ...
  return (
    <Container fluid className="mb-5">
      <Row>
        <SectionTitle title="افزودن کمیته" />
      </Row>
      <Row className="mt-3">
        <Col xs={3}>
          <label>عنوان :</label>
        ...
        <Col xs={3}>
          <Form.Label htmlFor="committeesImage">تصویر کمیته :</Form.Label>
          <Form.Control
            type="file"
            id="committeesImage"
            className="mt-1"
        ...
        <Col xs={12}>
          <Col xs={3} style={{"margin":"20px 0"}}>
            <Form.Label htmlFor="committeesFiles">فایل کمیته (pdf.) :</Form.Label>
            <Form.Control
              type="file"
              accept=".pdf"
              id="committeesFiles"
        ...
        <Col xs={12} className="text-start mt-4">
          <Button
            ...
            color="success"
            size="large"
            onClick={createCalendarsHandler}
          >
            ایجاد کمیته
          </Button>
        </Col>
